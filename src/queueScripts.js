...
function setQueueName() {
    ...
    $(`#songQueueResults`).empty();
    var message = `
        <div style="max-width: 30em; display: block;"><input id="queueName" type="text" style="margin: 1em;" class="form-control" placeholder="Enter a Name for the Queue"></div>
        <button type="button" class="btn" onclick="createQueue()">
            Create
        </button>
    `;

    $(`#queueResult`).html(message);
};
    ...
    api_call('https://api.spotify.com/v1/me').then( (res) => {
        ...
        $.ajax({
            ...
            success: function (result) {
                console.log(result); 
                console.log("SUCCESS");
                addCreatorAsOwner(result, queueObj['userId']);
                displayQueue(result);
            },
            error: function (err) {console.log("ERROR");}
        });     
...
function addCreatorAsOwner (queue_id, user_id) {
    console.log(queue_id);
    console.log(user_id);

    var accessObj = new Object();
    accessObj['userId'] = user_id;
...
function getUserQueues() {
    ...
    api_call('https://api.spotify.com/v1/me').then( (res) => {
        ...
        $.ajax({
            type: 'POST',
            url: 'getUserQueue.php',
            data: {'data': JSON.stringify(res['id'])},
            success: function (result) {
                document.getElementById("queueResult").innerHTML = result;
            },
            error: function (err) {console.log("ERROR");}
